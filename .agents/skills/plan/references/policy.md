...
- `plans/backlog/` for draft plans that are still being explored or awaiting approval to implement.
- `plans/active/` for plans that are approved and currently being implemented.
- `plans/completed/` for finished plans, with Outcomes & Retrospective filled in.
...
- When the ExecPlan is the only content of a file (e.g. in `plans/active/`), do not wrap the whole thing in triple backticks; the file is the plan.
...
Copy this skeleton into a new file in `plans/backlog/` named `YYYYMMDD-<title>.md` and fill it in. Move it to `plans/active/` when implementation is approved.
...
This plan lives in `<repo>/plans/` because <reason>.
